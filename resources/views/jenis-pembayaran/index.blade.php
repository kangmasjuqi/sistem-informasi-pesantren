...
<div class="card">
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover" id="jpTable" style="width: 100%;">
                <thead>
                    <tr>
                        <th>Kode</th>
                        <th>Nama</th>
...
@include('jenis-pembayaran.modal')
...
$(document).ready(function () {
    ...
    $('#jpForm').on('submit', function (e) {
        ...
        const id     = $('#jpId').val();
        const url    = id ? `/jenis-pembayaran/${id}` : '/jenis-pembayaran';
        ...
        const formData = {
            ...
            nominal:   $('#nominal').val().replace(/\./g, '').replace(',', '.'),
    ...
    $(document).on('click', '.btn-edit', function () {
        editJenisPembayaran($(this).data('id'));
    });
    ...
    // ── Format nominal input as Rupiah ───────────────────────────
    ...
    function initDataTable() {
        table = $('#jpTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '/jenis-pembayaran/data',
                data: function (d) {
            ...
            columns: [
                ...
                { data: 'nama' },
                {
                    ...
                    render: function (data) {
                        const map = {
                            bulanan:     { cls: 'kategori-bulanan',     label: 'Bulanan' },
                            tahunan:     { cls: 'kategori-tahunan',     label: 'Tahunan' },
                            pendaftaran: { cls: 'kategori-pendaftaran', label: 'Pendaftaran' },
                            kegiatan:    { cls: 'kategori-kegiatan',    label: 'Kegiatan' },
                            lainnya:     { cls: 'kategori-lainnya',     label: 'Lainnya' },
                        };
                        const item = map[data] ?? { cls: 'kategori-lainnya', label: data };
                        return `<span class="kategori-badge ${item.cls}">${item.label}</span>`;
                    },
                },
                {
    ...
    function editJenisPembayaran(id) {
        showLoading();
        $.ajax({
            url: `/jenis-pembayaran/${id}`,
            method: 'GET',
            success: function (res) {
                ...
                const d = res.data;
                $('#jpId').val(d.id);
                $('#modalTitle').text('Edit Jenis Pembayaran');
                $('#kode').val(d.kode);
                $('#nama').val(d.nama);
                $('#kategori').val(d.kategori);
                $('#nominal').val(parseFloat(d.nominal).toLocaleString('id-ID'));
