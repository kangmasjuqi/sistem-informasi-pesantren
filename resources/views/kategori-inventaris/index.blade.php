...
<div class="card">
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover" id="kiTable" style="width:100%;">
                <thead>
                    <tr>
                        <th>Kode</th>
                        <th>Nama Kategori</th>
...
@include('kategori-inventaris.modal')
...
$(document).ready(function () {
    ...
    $('#btnCreate').on('click', function () {
        resetForm();
        $('#modalTitle').text('Tambah Kategori Inventaris');
        $('#kiModal').modal('show');
    });
    ...
    // ── Form submit ──────────────────────────────────────────────
    $('#kiForm').on('submit', function (e) {
        e.preventDefault();

        const id     = $('#kiId').val();
        const url    = id ? `/kategori-inventaris/${id}` : '/kategori-inventaris';
    ...
    // ── Edit & Delete ────────────────────────────────────────────
    $(document).on('click', '.btn-edit',   function () { editKategori($(this).data('id')); });
    ...
    function initDataTable() {
        table = $('#kiTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '/kategori-inventaris/data',
                data: function (d) {
            ...
            columns: [
                ...
                { data: 'nama', render: d => `<strong>${d}</strong>` },
                {
                    data: 'deskripsi',
                    render: function (d) {
                        if (!d) return '<span class="text-muted" style="font-size:.8125rem;">—</span>';
                        return d.length > 70 ? `<span title="${d}">${d.substring(0,70)}…</span>` : d;
                    },
                },
                {
                    data: 'is_active',
                ...
                },
                {
                    ...
                    render: function (row) {
                        ...
                        <div style="display:flex; gap:.25rem;">
                            ...
                            <button class="btn btn-sm btn-danger btn-delete" data-id="${row.id}" data-nama="${row.nama}" title="Hapus">
                                <svg width="14" height="14" fill="currentColor" viewBox="0 0 16 16">
    ...
    function editKategori(id) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                const d = res.data;
                $('#kiId').val(d.id);
                $('#modalTitle').text('Edit Kategori Inventaris');
                $('#kode').val(d.kode);
                $('#nama').val(d.nama);
    ...
    function resetForm() { $('#kiForm')[0].reset(); $('#kiId').val(''); }
